...
export class Extractor implements RequestExtractor {
  ...
  constructor(
    ...
    logFactory: ComponentSpecificLoggerFactory
  ) {
  ...
  async fits(context: RequestContext): Promise<boolean> {
    this.logger.debug(
      ...
      "Checking request for dialogflow..."
    );
    ...
    if (
      typeof this.configuration.authenticationHeaders === "undefined" ||
      Object.keys(this.configuration.authenticationHeaders).length < 1
    ) {
      throw new Error(
      ...
      const headersAreValid =
        Object.keys(this.configuration.authenticationHeaders).filter(
          headerKey =>
            context.headers[headerKey] !==
              this.configuration.authenticationHeaders[headerKey] &&
            context.headers[headerKey.toLowerCase()] !==
              this.configuration.authenticationHeaders[headerKey] &&
            context.headers[headerKey.toUpperCase()] !==
              this.configuration.authenticationHeaders[headerKey]
        ).length === 0;
      ...
      if (headersAreValid) {
        this.logger.debug(
          { requestId: context.id },
          "Request matched for dialogflow."
        );
        return true;
      } else {
        this.logger.warn(
          { requestId: context.id },
          "Given headers did not match configured authenticationHeaders. Aborting."
        );
        return false;
      }
    }
    ...
    if (typeof request.result !== "undefined") {
      if (typeof request.result.parameters !== "undefined") {
        ...
        Object.keys(request.result.parameters).forEach(slotName => {
          if (
            typeof request.result.parameters[slotName] !== "undefined" &&
            request.result.parameters[slotName] !== ""
          )
  ...
  protected getGenericIntent(context: RequestContext): GenericIntent | null {
    return Extractor.makeIntentStringToGenericIntent(
      context.body.result.metadata.intentName
    );
  }
  ...
  static makeIntentStringToGenericIntent(intent: string): GenericIntent | null {
    return apiaiToGenericIntent.hasOwnProperty(intent)
      ? apiaiToGenericIntent[intent]
      : null;
  }
}
